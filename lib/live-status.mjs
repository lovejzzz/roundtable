...
export function livenessDetailText({
  ...
  if (state === "process-active") {
    ...
    return quiet >= 20
      ? `Process alive · ${elapsed} elapsed · no CLI output for ${formatLivenessDuration(quiet)}`
      : `Process alive · ${elapsed} elapsed`;
  }
  if (state === "process-exited") return `Process exited · collecting the result after ${elapsed}`;
  if (state === "broker-active") {
    return `Verification check running · ${elapsed} elapsed · the model reply is waiting for evidence`;
  }
  if (state === "request-active") return `Request open · ${elapsed} elapsed · provider progress is not observable`;
...
export function liveStatusText({
  ...
  if (status === "failed") {
    return `${agentLabel(failedRole)} could not complete turn ${Math.min(completed + 1, total || completed + 1)}. Retry, skip, or end the discussion.`;
  }
...
export function pendingSteeringPresentation({ terminal = false, archived = false } = {}) {
  if (terminal || archived) {
    return {
      title: "Queued, never delivered",
      description:
        "The bridge stopped before these notes reached another agent, so they are not part of the shared transcript.",
